...
userSchema.statics.findbyTokenAndValidate = function(token){
    ...
    return User.findOne({'tokens.token':token, 'tokens.type':'auth'})
    .then((user)=>{
        if(!user){
            console.log("msh l2eih fe3lan");
            ...
            let temptime=Date.now()-50;
            if(currentTime - temptime <= (60*24*60*1000)){
                return Promise.resolve(user);
            } else {
                console.log("el age el moshkela");
                return Promise.reject({
                    message:"Unauthorized"
                })
            }
